(function () {
    ...
    try {
        ...
        // 3. 剔除那些只是用来包裹整个对话列表的超大外壳（避免把整个历史记录里的多条回答合成一条抓进去）
        // 做法：如果一个节点包含了其它有效的消息气泡，极可能是最外层的 Dialog/Thread 壳子，我们应当只保留具体的条目。
        let finalNodes = [];
        validNodes.forEach(n => {
            let containsOther = validNodes.some(other => other !== n && n.contains(other));
            if (!containsOther) {
                finalNodes.push(n);
            }
        });

        // 此时 validNodes 存放的是最原子的一个个气泡（不被别人包含的正文段落可能会被并列，只要确保不是最高外壳就行）
        validNodes = finalNodes.length > 0 ? finalNodes : validNodes;
