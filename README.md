...
```text
WebViewHub/
├── App.xaml / App.xaml.cs              # 应用程序入口与全局配置
├── MainWindow.xaml / .cs               # 主窗口：核心画布布局、WebView 调度逻辑
├── FloatingIslandWindow.xaml / .cs     # 悬浮岛/球：独立置顶窗口、状态机动画逻辑
├── Controls/                           # 自定义复合 UI 控件 (UserControls)
│   ├── CentralCommandPanel             # 中央群发调度台（@mention, @ALL 广播）
│   ├── WebViewContainer                # 磁性吸附容器，管理单个 AI 网格状态
│   ├── WebViewUnit                     # WebView2 实例封装，处理浏览器行为
│   ├── ClearCacheDialog                # 内置缓存/Cookie 一键清理工具
│   ├── AppleRoleDialog                 # 角色提取与 @ 功能配置弹窗
│   ├── AppleUrlDialog                  # 自定义 AI 网站地址配置弹窗
│   └── AppleMessageBox                 # 统一定制的苹果风格提示弹窗
├── Services/                           # 后台业务逻辑与单例服务
│   ├── AppLogger.cs                    # 结构化日志系统（文件+调试输出）
│   ├── AppDatabase.cs                  # SQLite 核心服务，处理所有持久化数据
│   └── LayoutService.cs                # 布局与窗口参数的持久化逻辑
├── Styles/                             # UI 样式与主题资源
│   └── AppStyles.xaml                  # 毛玻璃效果、控件颜色、圆角定义
├── Assets/                             # 静态资源文件
│   ├── lucky_cat.png / praying.png     # 祈祷/幸运动画素材
│   └── alipay.jpg / wechat_pay.jpg     # 捐赠二维码素材
├── extractor.js                        # 核心组件：跨平台 DOM 提取引擎
├── turndown.js                         # 核心组件：HTML 逆向 Markdown 转化引擎
